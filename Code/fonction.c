...
//Fonctions données
void stiffnessMatrix ( double x1 , double y1 , 
	double x2 , double y2 , double ea , double k [ 4 ] [ 4 ] )
{
	...
	double *f , int *nc , double *vc , double *ea , double *x, double *r ) 
{
	...
	printf("stiffnessMatrix and add_to_matrix\n");
	for ( int i=0; i<B ; i++) 
	{
		int n1 = ind[2*i] ;
		int n2 = ind[2*i+1] ;
		...
		stiffnessMatrix ( xy[indx[0]] , xy[indx[1]] , xy[indx[2]] , xy[indx[3]] , ea[i] , stiff ) ;
